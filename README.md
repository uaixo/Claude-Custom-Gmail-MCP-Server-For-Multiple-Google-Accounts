...
- **Transient errors:** Gmail calls are retried automatically with bounded, jittered backoff. Read/label operations retry on rate limits (429) and transient server errors (5xx). Sending and draft creation retry **only on 429** (a rejected-before-processing rate limit) so a transient 5xx can't deliver a duplicate; other failures surface immediately.
