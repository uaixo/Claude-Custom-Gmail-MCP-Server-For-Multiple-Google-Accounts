...
| `gmail_get_attachment` | Download a message attachment — inline (base64) or saved into `GMAIL_MCP_ATTACHMENTS_DIR` |
...
3. Configure the **OAuth consent screen** (APIs & Services → OAuth consent screen):
   ...
   - Save. **Publishing status matters:** while the app is in "Testing" status, Google **expires every sign-in after 7 days** for apps requesting Gmail scopes — all connected accounts fail with `invalid_grant` weekly and must be re-connected with `npm run add-account`. For personal long-term use, click **Publish app** ("In production"): you do NOT need Google's verification review — sign-in shows an "unverified app" warning you can click through (Advanced → continue), and refresh tokens then last until revoked. Stay in "Testing" only if you're fine re-consenting every week.
...
- **Transient errors:** Gmail calls are retried automatically with bounded, jittered backoff (honoring a server-sent `Retry-After`, capped), and each request has a timeout (`GMAIL_MCP_REQUEST_TIMEOUT_MS`, default 30s) so a stalled socket fails fast instead of hanging. Read operations and label *edits* retry on rate limits (429 and Gmail's rate-limit 403s), transient server errors (5xx), and transport failures (timeout/connection reset). Sending, draft creation, and label **creation** retry **only on rate limits** (rejected before processing) so a transient 5xx — or a timeout that may actually have been delivered/committed — can't produce a duplicate or mask a success as a failure; other failures surface immediately.
- **Attachments (sending):** supplied per call via local `path` or inline `content_base64`. Reading by `path` is disabled unless `GMAIL_MCP_ATTACHMENTS_DIR` allowlists the directory it lives in (a guardrail against emailing arbitrary local files); inline base64 is impractical for large binaries the model can't see, so set the allowlist and use `path` for those. Gmail's total message size limit (~25 MB) applies.
- **Attachments (receiving):** `gmail_get_thread` / `gmail_get_message` list each message's attachments (`attachment_id`, `filename`, `mime_type`, `size`); `gmail_get_attachment` downloads one — small files inline as base64, larger ones saved into the first `GMAIL_MCP_ATTACHMENTS_DIR` directory (sender-controlled filenames are sanitized to a safe basename, and name collisions get a ` (n)` suffix instead of overwriting). Without the allowlist configured, only inline downloads are available.
...
- **Token expiry:** access tokens refresh automatically. If a refresh token is revoked (password change, manual revocation, long inactivity on an unverified app, or the **7-day automatic expiry when your OAuth app is left in "Testing" status** — see the setup note above), re-run `npm run add-account` for that account. A running server detects the rewritten credentials and picks them up on its next call — no restart needed.
- **Multiple OAuth clients:** drop several `credentials*.json` files in the data dir; each account records which one authorized it and refreshes with that same client. Don't remove a credential file an account still depends on (the server returns an actionable error if it's missing). Legacy `tokens.json` files from before this feature are migrated automatically and assumed to use the default credential file (`credentials.json` in the data dir, or the file `GMAIL_OAUTH_CREDENTIALS` points at if set).
