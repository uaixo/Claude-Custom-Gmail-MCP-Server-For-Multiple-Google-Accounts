...
import { buildRawMessage, extractPlainText, gmailFor, handleGmailError, header, resolveAttachments, } from "./gmail.js";
...
const attachmentSchema = z
    ...
    filename: z
        .string()
        .optional()
        .describe("Display filename. Defaults to the basename for 'path'; required for 'content_base64'."),
    path: z
        .string()
        .optional()
        .describe("Absolute or relative path to a local file on the machine running the server."),
    content_base64: z
        .string()
        .optional()
...
const accountField = z
    ...
    .email()
    .optional()
    .describe("Email address of the connected Gmail account to use. Optional when only one account is connected; required to disambiguate when several are.");
...
// ---------------------------------------------------------------------------
server.registerTool("gmail_search_threads", {
...
Returns: JSON {
  ...
  "messages": [ { "message_id": string, "from": string, "to": string, "date": string, "subject": string, "body": string, "label_ids": string[] } ]
    ...
    try {
        ...
        const text = capText(JSON.stringify(output, null, 2), "Thread body was large; consider reading messages individually.");
        return { content: [{ type: "text", text }], structuredContent: output };
    }
    catch (error) {
        return { content: [{ type: "text", text: handleGmailError(error) }] };
...
}, async ({ to, subject, body, is_html, attachments, cc, bcc, account, thread_id, in_reply_to, }) => {
    try {
        const { gmail, account: acct } = gmailFor(account);
        const resolvedAttachments = resolveAttachments(attachments);
        const raw = buildRawMessage({
...
async function main() {
    ...
    const accounts = listAccounts();
    console.error(`gmail-mcp-server running via stdio. Connected accounts: ${accounts.length ? accounts.join(", ") : "none (run `npm run add-account`)"}`);
}
main().catch((error) => {
